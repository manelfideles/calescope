...
const sbTokenName =
  'sb-' +
  sbUrl.substring(indexOf(sbUrl, '/') + 2, indexOf(sbUrl, '.')) +
  '-auth-token';

export const AuthContext =
  createContext<AuthContextInterface>(initialAuthState);
...
export const AuthContextProvider = ({ children }: AuthProviderProps) => {
  ...
  const [authState, setAuthState] = useState<AuthState>({
    session: localStorage.getItem(sbTokenName)!
      ? JSON.parse(localStorage.getItem(sbTokenName)!)
      : null,
    user: localStorage.getItem(sbTokenName)!
      ? JSON.parse(localStorage.getItem(sbTokenName)!).user
      : null,
